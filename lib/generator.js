...
function generator(root, config){
  ...
  var pathMap = {
	...
	var classStr = primitiveHandler(importedConfig, pathMap["module"], visRoot);
	//log.debug(classStr);
	var panelStr = panelHandler(importedConfig);
	//log.debug(panelStr);

	fs.writeFileSync(moduleWritePath, classStr);
	fs.writeFileSync(path.join(pathMap.panel, importedConfig.name.toLowerCase() + ".js"), panelStr);
